      ...
      const deleteUserSubscription = this._service.deleteUser(this.userId).subscribe(
        (data: any) => {
          ...
          if (data && data.result === 1) {
            this._toast.success({ 
              detail: "SUCCESS", 
              summary: data.data || 'User deleted successfully',
            ...
            this._toast.error({ 
              detail: "ERROR", 
              summary: data?.message || 'Failed to delete user',
              duration: APP_CONFIG.toastDuration 
            });
          }
        },
        (error: any) => {
          ...
          console.error('Delete user error in component:', error);
          
          // Try to get more detailed error messages
          let errorMessage = error.error?.message || 
            error.message || 
            `HTTP Error ${error.status || 'Unknown'}: ${error.statusText || 'Unknown error'}` || 
